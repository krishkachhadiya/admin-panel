  ...
  async function fetchCategory() {
    try {
      ...
      if (category) {
        setFormData({
          ...category,
          parent: category.parent ? Number(category.parent) : null,
        });

        const filteredSubcategories = data.filter((item) => {
          return (
            item.parent !== null &&
            String(item.parent) === String(params.id)
          );
        });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-xl p-10">
        {/* Header */}
        <div className="mb-10">
          <h1 className="text-5xl font-bold text-gray-900">Edit Category</h1>
          <p className="text-gray-500 mt-3 text-lg">Update category details</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const exists = allCategories.some(
                  (item) => item.id != params.id && item.slug === slugValue
                );
          ...
          <div className="text-black">
            ...
            <CategoryPicker
              categories={allCategories.filter((item) => item.id !== formData.id)}
          ...
          {formData.parent === null && (
            <div className="text-black">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-2xl font-bold text-black">Subcategories</h2>
                <button
                  type="button"
                    ...
                    {subcategories.length > 0 ? (
                      subcategories.map((subcategory) => (
                        <tr key={subcategory.id} className="border-b">
                          <td className="p-4">{subcategory.title}</td>
                          <td className="p-4">{subcategory.slug}</td>
                          <td className="p-4">{subcategory.status}</td>
                          <td className="p-4">
