  ...
  async function fetchRoles() {
    try {
      ...
      if (data.roles?.length > 0) {
        setSelectedRole(String(data.roles[0].id));
      }
    } catch (error) {
      console.log(error);
    } finally {
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-2">Manage roles & permissions</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-x-auto">
        <table className="w-full min-w-[700px] border-collapse">
          ...
          <thead className="bg-black text-white">
            <tr>
              <th className="text-left p-5 border">Module</th>
              <th className="text-center p-5 border">Permissions</th>
              <th className="text-center p-5 border">Actions</th>
            </tr>
          </thead>
              ...
              <tr key={module} className="hover:bg-gray-50 transition">
                ...
                <td className="p-5 border">
                  <div className="flex flex-wrap gap-2 justify-center">
                    {selectedRoleData?.permissions?.[module]?.create && (
                      <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm">
                        Create
                      </span>
                    )}

                    {selectedRoleData?.permissions?.[module]?.edit && (
                      <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm">
                        Edit
                      </span>
                    )}

                    {selectedRoleData?.permissions?.[module]?.delete && (
                ...
                <td className="p-5 border">
                  <div className="flex justify-center gap-3">
                    <button
                      onClick={() =>
                        router.push(`/admin/roles/edit/${selectedRole}`)
                      }
