...
export default function EditCMSPage() {
  ...
  async function fetchPage() {
    try {
      const response = await fetch(`/api/cms/${params?.id}`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      // FETCH EXISTING CMS
      const cmsResponse = await fetch("/api/cms");
      const existingCms = await cmsResponse.json();
      ...
      // TITLE EXISTS
      // ======================
      const titleExists = existingCms.some(
        (item) =>
      ...
      if (titleExists) {
        alert("CMS title already exists");
        return;
      }
    ...
    <div className="min-h-screen bg-gray-100 p-10">
      <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-xl p-10">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500 mt-3 text-lg">Update website page</p>
        </div>
